(function (exports, d3) {
    ...
    class Markmap {
      ...
      fetchRandom(hashdata, data)  {
        ...
        const hashrand =  letters.slice(0, n);
            data.children = [];
      ...
      toggleNode(hashdata, data) {
        ...
        if(!_data$fold && !data.hasChild){
          data.hasChild = true;
          this.fetchRandom(hashdata, data);
      ...
      static create(svg, hashdata, diagStr) {
        const mm = new Markmap(svg, null);
        mm.setDataHash(hashdata, diagStr);
        mm.fit();
        return mm;
      }
    }
